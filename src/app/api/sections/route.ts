...
async function canUserEditCourse(userId: string, courseId: string): Promise<boolean> {
  ...
  try {
    ...
    if (roleNames.some(r => ['SUPER_ADMIN', 'ADMIN', 'ACADEMIC_ADMIN'].includes(r))) {
      return true;
    }
  } catch (err) {
    console.error('[canUserEditCourse] Error checking roles:', err);
  }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // ONLY the course owner or admin can delete sections (collaborators CANNOT delete)
    if (course.instructor_id !== user.id && !isAdmin) {
      return NextResponse.json({ 
        error: 'Action non autorisée : seuls le propriétaire du cours et les administrateurs peuvent supprimer des sections.' 
      }, { status: 403 });
    }
