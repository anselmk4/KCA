...
export async function GET(req: NextRequest, { params }: { params: Promise<{ paymentId: string }> }) {
  try {
    ...
    const safeAcademyTagline = escapeHtml(academyTagline);
    ...
    const safeTxId = escapeHtml(payment.provider_transaction_id);
...
<head>
  ...
  <title>Facture ${safeInvoiceNumber} — ${isInstructorInvoice ? 'ANSELLA' : safeAcademyName}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .item-title { font-weight: 600; color: #0f172a; margin-bottom: 2px; }
    .item-desc { font-size: 12px; color: #64748b; }
    .totals { margin-left: auto; width: 280px; }
    .total-row { display: flex; justify-content: space-between; font-size: 13px; color: #64748b; margin-bottom: 8px; }
    .total-row.grand {
      font-size: 18px;
      font-weight: 800;
      color: #0f172a;
...
<body>
  <div class="invoice">
    <div class="header">
      <div class="logo-container">
        <div class="ansella-logo">
          <span style="font-size: 20px; font-weight: 900; letter-spacing: -0.5px; color: #38bdf8;">ANSELLA<span style="color: #f8fafc;">.</span></span>
        </div>
        ${!isInstructorInvoice ? `
        <div class="academy-info">
          <div class="brand-name">${safeAcademyName}</div>
          <div class="brand-tagline">${safeAcademyTagline}</div>
        </div>
        ` : ''}
      </div>
      <div>
    ...
    <div class="body">
      <div class="meta-grid">
        <div class="meta-block">
          ...
          <p>${safeUserName}</p>
          <p class="light">${safeUserEmail}</p>
        </div>
        <div class="meta-block">
          <label>Informations de paiement</label>
          <p>Date : ${escapeHtml(invoiceDate)}</p>
          <p class="light">Mode : ${safeProviderLabel}</p>
          ${safeTxId ? `<p class="light" style="font-size:11px;margin-top:4px">Réf. : ${safeTxId}</p>` : ""}
        </div>
      </div>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              <div class="item-title">${safeItemLabel}</div>
              ${safeItemDescription ? `<div class="item-desc">${safeItemDescription}</div>` : ""}
            </td>
            <td>1</td>
            <td>${safeAmount} ${safeCurrency}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="totals">
        <div class="total-row">
          ...
          <span class="amount">${safeAmount} ${safeCurrency}</span>
        </div>
        <div class="total-row">
          <span>TVA (0%)</span>
          <span class="amount">0,00 ${safeCurrency}</span>
        </div>
    ...
    <div class="footer">
      <div>
        <p>${isInstructorInvoice ? 'ANSELLA' : `${safeAcademyName} (ANSELLA)`} — info@ansella.app</p>
