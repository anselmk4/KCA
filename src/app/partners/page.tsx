      ...
      <main className="flex-1 py-20">
        ...
        <div className="container mx-auto px-4 md:px-8 max-w-6xl space-y-16 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            <div className="bg-zinc-950/40 border border-zinc-900 rounded-2xl p-6 text-left space-y-3">
              <ShieldCheck className="w-8 h-8 text-teal-400" />
              <h3 className="font-bold text-white text-base">Sécurité & Fiabilité</h3>
              <p className="text-xs text-zinc-400 leading-relaxed">Des certifications de classe mondiale et des transactions Mobile Money réglementées.</p>
            </div>
            <div className="bg-zinc-950/40 border border-zinc-900 rounded-2xl p-6 text-left space-y-3">
              <Cpu className="w-8 h-8 text-teal-400" />
              <h3 className="font-bold text-white text-base">Technologie & Web3</h3>
              <p className="text-xs text-zinc-400 leading-relaxed">Intégration poussée de l&apos;intelligence artificielle et des architectures de smart-contracts.</p>
            </div>
            <div className="bg-zinc-950/40 border border-zinc-900 rounded-2xl p-6 text-left space-y-3">
              <Target className="w-8 h-8 text-teal-400" />
              <h3 className="font-bold text-white text-base">Impact Local</h3>
              <p className="text-xs text-zinc-400 leading-relaxed">Des opportunités concrètes et adaptées aux réalités économiques de la RDC.</p>
            </div>
          </div>
          ...
          <div className="space-y-8 pt-8">
            <div className="text-left border-b border-zinc-800 pb-4">
              ...
              <p className="text-xs text-zinc-500 mt-1">Découvrez comment chacun de nos partenaires contribue à la réussite des créateurs Ansella.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {partnersList.map((partner, index) => (
                <div 
                  key={index} 
                  className={`bg-gradient-to-br ${partner.color} bg-zinc-950/30 border border-zinc-850 hover:border-zinc-700/80 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-start gap-6 hover:shadow-lg transition-all ${partner.border} duration-300 text-left group`}
                >
                  {/* Logo block */}
                  <div className="shrink-0 w-20 h-20 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform duration-300">
                    {partner.logo}
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <h3 className="font-bold text-lg text-white group-hover:text-teal-400 transition-colors leading-snug">{partner.name}</h3>
                      <p className="text-xs text-teal-500 font-semibold">{partner.role}</p>
                    </div>
          ...
          {/* CTA Banner */}
          <div className="bg-gradient-to-br from-zinc-950 to-zinc-900 border border-zinc-800 rounded-3xl p-8 md:p-12 text-center max-w-4xl mx-auto space-y-6 relative overflow-hidden">
            ...
            <a 
              ...
              className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 text-white font-bold text-xs transition-colors"
            >
              Nous contacter par email <ArrowRight className="w-4 h-4 text-teal-400" />
            </a>
          </div>
