    ...
    <div className="flex flex-col bg-zinc-50 dark:bg-zinc-950 min-h-screen text-zinc-900 dark:text-zinc-100">
      ...
      <main className="flex-1 py-12 px-4 md:px-8">
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-300 pb-16">
          ...
          {/* Dashboard Back Link if logged in */}
          {currentUser && (
            <div className="flex justify-end mb-2">
              <Link
                href={
                  currentUserRole === "SUPER_ADMIN" || currentUserRole === "ADMIN" || currentUserRole === "FINANCE_ADMIN" || currentUserRole === "ACADEMIC_ADMIN" || currentUserRole === "SUPPORT_AGENT"
                    ? "/admin"
                    : currentUserRole === "INSTRUCTOR" || currentUserRole === "TEACHING_ASSISTANT"
                    ? "/instructor"
                    : "/dashboard"
                }
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-zinc-205 dark:bg-zinc-800 hover:bg-zinc-200 text-xs font-bold text-zinc-700 dark:text-zinc-300 transition-colors"
              >
                Retourner à mon Espace (Dashboard)
              </Link>
            </div>
          )}
          ...
          <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-teal-600 via-teal-700 to-emerald-800 shadow-xl">
            ...
            <div className="relative p-8 md:p-10 flex flex-col md:flex-row gap-6 items-start md:items-center">
              ...
              {isInstructor && (
                <div className="flex gap-6 shrink-0 text-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-3">
                    {paidCourses.map(c => (
                      <div key={c.id} className="flex items-center justify-between p-4 rounded-xl border border-zinc-100 dark:border-zinc-800">
                        ...
                        <span className="text-xs text-emerald-600 font-semibold bg-emerald-50 dark:bg-emerald-950/20 px-2 py-1 rounded-lg">Payé</span>
                      </div>
                    ))}
                  </div>
                </div>
                            ...
                            {currentUser && (
                              <div className="flex items-center gap-2 mt-2">
                                <input
                                  ...
                                  className="flex-1 px-3 py-2 text-xs rounded-xl bg-zinc-50 dark:bg-zinc-850 border border-zinc-200 dark:border-zinc-700 focus:border-teal-500 outline-none text-zinc-900 dark:text-white"
                                />
                                <button
            ...
            <div className="space-y-5">
              ...
              {/* Edit profile & dashboard shortcut (own profile) */}
              {isOwn && (
                <div className="space-y-3">
                  <Link
                  ...
                  <Link
                    ...
                    className="flex items-center justify-center gap-2 w-full py-3 rounded-2xl bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-350 text-zinc-900 dark:text-white font-semibold text-sm transition-all shadow-md"
                  >
